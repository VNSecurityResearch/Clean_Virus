...
int clearShellCode(const string directory) {
...
int searchFile(const char * dirn)
{
	...
	do
	{
		...
		if (f.dwFileAttributes&FILE_ATTRIBUTE_DIRECTORY)
		{
			...
			string fn = name;
			if (fn.substr(fn.find_last_of(".") + 1) == "exe" || fn.substr(fn.find_last_of(".") + 1) == "crv") {
				killProcessByName(name);
				int s = clearShellCode(filePath);
				clear_done += s;
				cutfile(filePath);
				cout << filePath << endl;
			}
...
int main()
{
	...
	int clear_ok = searchFile(directory);
	cout << "Clear done! Restore complete " << clear_done << " program" << endl;
	system("pause");
}